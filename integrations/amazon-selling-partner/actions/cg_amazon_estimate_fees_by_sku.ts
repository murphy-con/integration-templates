...
const action = createAction({
    description: 'Estimate Amazon fees for one seller SKU using an explicit price and fulfillment mode.',
    version: '1.0.0',
    input: InputSchema,
    output: OutputSchema,
    scopes: [],
    exec: async (nango, input) =>
        ...
        return (OutputSchema.parse(
            await postJson(nango, `/products/fees/v0/listings/${encodeURIComponent(input.sku)}/feesEstimate`, {
                FeesEstimateRequest: {
