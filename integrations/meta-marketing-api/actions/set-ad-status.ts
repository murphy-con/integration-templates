...
const InputSchema = z.strictObject({ ad_id: Id, status: z.enum(['ACTIVE', 'PAUSED']) });
const OutputSchema = z.object({ success: z.boolean(), id: Id.optional() });

export default createAction({
    description: 'Activate or pause one Meta ad.',
    version: '1.0.0',
    input: InputSchema,
    output: OutputSchema,
    ...
    exec: async (nango, rawInput): Promise<z.infer<typeof OutputSchema>> => {
        ...
        const response = await nango.post({ retries: NO_WRITE_RETRIES, endpoint: `/v26.0/${input.ad_id}`, data: { status: input.status } });
